...
to initialize-sizes
  if fitness_function = "Fitness function 2"
    [
      set max-x 512
      set max-y 512
      resize-world (- max-x) max-x (- max-y) max-y
      set-patch-size 0.25
      set turtle-size 30
  ]
  ...
  if fitness_function = "Fitness function 7"
    [
      set max-x 100
      set max-y 100
      resize-world (- max-x) max-x (- max-y) max-y
...
to initialize-topology
  ...
  ask patches [
     if fitness_function = "Fitness function 2"
       [set val fitness_function_2 pxcor pycor]

     if fitness_function  = "Fitness function 3"
       [set val fitness_function_3 pxcor pycor]

     if fitness_function = "Fitness function 7"
       [set val fitness_function_7 pxcor pycor]
  ]
...
; TODO dummy random fitness function to be implemented by students
to-report fitness_function_2 [x y]
  let x1 (x * sin(sqrt(abs(x)) * 180 / pi))
  let y1 (y * sin(sqrt(abs(y)) * 180 / pi))
  report 418.9829 * 2 - (x1 + y1);
end

; TODO dummy random fitness function to be implemented by students
to-report fitness_function_3 [x y]
  report (1 * cos ( 2 * x + 1 ) + 2 * cos ( 3 * x + 2 ) + 3 * cos ( 4 * x + 3 ) + 4 * cos ( 5 * x + 4 ) + 5 * cos ( 6 * x + 5 ) ) * ( 1 * cos ( 2 * y + 1 ) + 2 * cos ( 3 * y + 2 ) + 3 * cos ( 4 * y + 3 ) + 4 * cos ( 5 * y + 4 ) + 5 * cos ( 6 * y + 5 ) ) ;
end

; TODO dummy random fitness function to be implemented by students
to-report fitness_function_7 [x y]
  report (x + 2 * y - 7) ^ 2 + (2 * x + y - 5) ^ 2
end


; TODO: dummy random constraint to be implemented by students
...
to-report constraint_10 [x y]
  ifelse ((x * y) mod 90) = 0 or tan(x * y) < 1
  [report TRUE]
  [report FALSE]
end
...
fitness_function
fitness_function
"Fitness function 2" "Fitness function 3" "Fitness function 7"
1
...
Constraint
Constraint
"Constraint 3" "Constraint 5" "Constraint 10"
1
